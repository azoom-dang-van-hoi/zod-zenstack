...
import {
  ...
  getFieldSchemaDefault,
} from "./utils/schema-gen"
import {
  hasConstantVariable,
  isIgnoreModel,
  getIgnoreModels,
...
export async function generate(
  ...
  const excludeModels = [
    ...getExcludedModels(model, options),
    ...getIgnoreModels(
      model.declarations.filter((d): d is DataModel => isDataModel(d))
    ),
  ]
  ...
  const inputObjectTypes =
    prismaClientDmmf.schema.inputObjectTypes.prisma.filter(
      (type) =>
        !excludeModels.find((e) =>
          type.name.toLowerCase().startsWith(e.toLocaleLowerCase())
        )
    )
  const outputObjectTypes =
    prismaClientDmmf.schema.outputObjectTypes.prisma.filter(
      (type) =>
        !excludeModels.find((e) =>
          type.name.toLowerCase().startsWith(e.toLowerCase())
        )
    )
  ...
  await generateEnumSchemas(
    ...
    (prismaClientDmmf.schema.enumTypes.model ?? []) as DMMF.SchemaEnum[],
    project,
    model
  )
  ...
  // constants
  await generateConstantSchemas(project, model, output, excludeModels)
...
function getExcludedModels(model: Model, options: PluginOptions) {
  ...
  if (generateModels) {
    if (options.modelOnly === true) {
      ...
      const todo = getDataModels(model).filter((dm) =>
        generateModels.includes(dm.name)
      )
...
async function generateConstantSchemas(
  project: Project,
  zmodel: Model,
  output: string,
  excludedModels: string[]
  ...
  const constants: any[] = []
  for (const dm of zmodel.declarations.filter((d): d is DataModel =>
    isDataModel(d)
  )) {
    for (const attribute of dm.attributes) {
      if (attribute.decl.$refText === "@@defineConstant") {
        const constantName = getConstantValue(attribute.args[0].value)
        constants.push(
          `export const ${constantName} = ${getConstantValue(attribute.args[1].value)}`
        )
      }
    }
  }
  ...
  project.createSourceFile(
    path.join(output, "constants", "index.ts"),
    constants.join("\n"),
    { overwrite: true }
  )
}

function getConstantValue(express: Expression): any {
  switch (express.$type) {
    case "StringLiteral":
    case "BooleanLiteral":
    case "NullExpr":
    ...
    case "ObjectExpr":
      return (
        "{\n" +
        express.fields.reduce((acc, field) => {
          return acc + `${field.name}: ${getConstantValue(field.value)},\n`
        }, "") +
        "}"
      )

    case "ArrayExpr":
      return `[${express.items.map((item) => getConstantValue(item)).join(", ")}]`
    case "InvocationExpr":
      if (express.function.$refText !== "getConstantValue") {
        throw new Error(`Unsupported function ${express.function.$refText}`)
      }
      //@ts-ignore
      const constantName = express.args[0].value?.value as string
      //@ts-ignore
      const constantValue = express.args[1].value?.value as string
      return `${constantName.replace(/\'/g, "")}.${constantValue.replace(/\'/g, "")}`
    default:
      break
  }
}
...
async function generateModelSchema(
  ...
  sf.replaceWithText((writer) => {
    ...
    const relations = model.fields.filter((field) =>
    ...
    // 1. Model schema
    ////////////////////////////////////////////////
    const fieldsWithoutDefault = scalarFields.filter(
      (f) => !getFieldSchemaDefault(f)
    )
    // mark fields without default value as optional
    let modelSchema = makePartial(
      "baseSchema",
      fieldsWithoutDefault.length < scalarFields.length
        ? fieldsWithoutDefault.map((f) => f.name)
        : undefined
    )

    // omit fields
    const fieldsToOmit = scalarFields.filter((field) =>
      hasAttribute(field, "@omit")
    )
